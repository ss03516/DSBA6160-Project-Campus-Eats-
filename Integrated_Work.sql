...
INSERT INTO `driver` (`driver_id`, `student_id`, `license_number`, `date_hired`) VALUES (1,110,'fi3980','1989-01-18'),
(2,98,'js4003','2017-03-15'),(3,104,'xt8429','2002-10-11'),(4,43,'yv6392','2010-02-23'),
(5,75,'cf3679','1997-04-06'),
(6,68,'rx3942','1970-03-24'),
(7,143,'mf2647','2014-05-06'),
(8,96,'ia9372','2011-06-25');
...
CREATE TABLE IF NOT EXISTS `rating` (
  ...
  `restaurant_rating` INT(1) DEFAULT NULL,
  PRIMARY KEY (`order_id`),
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE AVG_DRIVER_RATING  ()
BEGIN  
    SELECT driver.driver_id, AVG(rating.driver_rating) as avg_drv_rating
    FROM driver
    INNER JOIN rating
	ON rating.driver_id = driver.driver_id
    Group By driver_id;
END // 
DELIMITER ;
...
# Rating system
#Restaurant
#Create a stored procedure to calculate ratings
DELIMITER //
...
BEGIN
UPDATE driver
SET driver.rating = (select (s.r+s.ro)/2  
from 
(Select AVG(rating.driver_rating) as r, driver.rating as ro
from rating 
left join driver 
on driver.driver_id = rating.driver_id 
where rating.driver_id = driver_id  
group by rating.driver_id) as s);
END //
DELIMITER // 
